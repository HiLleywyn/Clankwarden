...
| Moderation audit log (actor id, target id, action summary) | `mod_log_events` | Tamper-evident record of moderation actions (`.modlog`) |
| Dehoist actions (user id, action) | `dehoist_events` | Audit of automatic impersonation handling |

Clankwarden does **not** store presence, full channel transcripts, message
content for users who were never contained, emails, IPs, or any payment data.
...
- **Per user:** a server moderator can run `.modlog forget @user` to erase a
  ...
  warnings, dehoist and case data). Tamper-evident audit-log entries are
  retained for integrity; an operator can erase those too on request.
