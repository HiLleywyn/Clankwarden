...
- **Level-based containment.** A clanker is now placed at a level (L1..L5)
  matching their threat instead of one flat escape room. **L1 (Orientation)** is
  the lowest tier -- a short, friendly education gate (read the server rules /
  etiquette / scam safety, answer one question, walk free). **L5 (Clankermax)**
  is the highest tier, the full gauntlet reserved for confirmed scammers. To get
  out, a clanker **clears each level's trial to step DOWN one level**; clearing
  **Level 1 releases them**. **Failing a level's gate escalates them UP one
  level** (toward L5 / Clankermax) and adds **rust**, which lengthens the
  higher-level reflection waits -- a cycle of rust. A Clankermax must clear all
  five.
- **Auto-assessed entry level.** Scam-username / celebrity-impersonation /
  CCI-100% joins and cluster cleaves enter at **L5** (and gain the Clankermax
  role); hunter reports and clutch sweeps at **L4**; AutoMod hits and
  dehoist/impersonation at **L3**. Manual clanks default to **L3**; a mod may
  set the level: `.clank @user 5 reason`, the `/clank` `level` option, or move
...
- **One new role: Clankermax.** Layered on top of the base Clanker role at L5
  ...
  on dropping below it or on release. Auto-created by name if unset; configurable
  via `CLANKERMAX_ROLE_ID`.
- **Fun to watch.** A public **Tank Board** (`.clank tank`) shows every
  clanker's level, a rust meter, and a level ladder; short escalate/clear
  ...
  (`clanktank` category: placed / level-up / level-down / released).
- **Every current clanker starts at L3.** The 0304 migration backfills all
  existing containment records to **L3** (STANDARD CONTAINMENT), and members
  already wearing the Clanker role from a previous bot are adopted into the
  system at **L3** on startup (and via `.clank import`). Legacy imports have no
  stored roles, so a configurable default role (`CLANK_DEFAULT_ROLE_ID`, falling
  back to a role named User/Member) is registered as their restore set -- handed
  back only when they are unclanked.
- `.clank info` now shows level, entry level, rust, and Clankermax status.
